...
function renderGanttItem(params, api) {
    ...
    var text = (barLength > flightNumberWidth + 40 && x + barLength >= 180)
        ? flightNumber : '';
    // 自定义颜色数组
    var colors = ['green', 'yellow', 'blue', 'red', '#04a']
    // console.log(api.value(10))
    var color = colors[api.value(10)]
    ...
    // });
    // 自定义添加
    var rectAdd = clipRectByRect(params, {
        x: x, y: y, width: (barLength) / 4, height: barHeight
    });
